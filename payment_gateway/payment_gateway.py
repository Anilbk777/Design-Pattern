...
class PaymentGateway(ABC):
    ...
    def __init__(self):
        self.banking_system: Optional[BankingSystem] = None
...
class PaytmGateway(PaymentGateway):
    ...
    def validate_payment(self, payment_request: PaymentRequest) -> bool:
        ...
        if payment_request.amount <= 0 or payment_request.currency != "NPR":
            return False
        return True

    def initiate_payment(self, payment_request: PaymentRequest) -> bool:
        print(
            f"[Paytm] initiating payment of {payment_request.amount} {payment_request.currency} for {payment_request.sender}"
        )
...
class PaymentGatewayProxy(PaymentGateway):
    ...
    def process_payment(self, payment_request: PaymentRequest) -> bool:
        ...
        for attempt in range(self.max_retries + 1):
            if attempt > 0:
                print(
                    f"[Proxy] Retrying payment (attempt - {attempt}) for {payment_request.sender}."
                )
        ...
        if not result:
            print(
                f"[Proxy] Payment failed after {self.max_retries} attempts for {payment_request.sender}"
            )
...
class GatewayFactory:
    def get_gateway(self, type: GatewayType) -> PaymentGateway:
...
class PaymentController:
    ...
    def handle_payment(
        self, gateway_type: GatewayType, payment_request: PaymentRequest
    ):
        payment_gateway = GatewayFactory().get_gateway(gateway_type)
        service = PaymentService()
        service.set_gateway(payment_gateway)
        return service.process_payment(payment_request)


if __name__ == "__main__":
    payment_request1 = PaymentRequest("Ram", "shyam", 500, "NPR")

    print("Processing via Paytm")
    print("=" * 60)
    payment_controller = PaymentController()
    result1 = payment_controller.handle_payment(GatewayType.PAYTM, payment_request1)
    ...
    print("=" * 60)
    payment_controller = PaymentController()
    result2 = payment_controller.handle_payment(GatewayType.RAZORPAY, payment_request2)
